...
TrajectoryPlannerWaypoints::TrajectoryPlannerWaypoints() {}

TrajectoryPlannerWaypoints::~TrajectoryPlannerWaypoints() {}


Eigen::MatrixXd TrajectoryPlannerWaypoints::minJerkTrajGenerate(
        const Eigen::MatrixXd &path_M,  //3d waypoints coordinates nx3
        const Eigen::MatrixXd &vel_M,   //boundary velocity 2x3
        const Eigen::MatrixXd &acc_M,   //boundary acceleration 2x3
    ...
    int segments = time_V.size();  //the number of segments
    ...
    b.row(6 * segments - 3) = path_M.row(segments);
    b.row(6 * segments - 2) = vel_M.row(1);
    b.row(6 * segments - 1) = acc_M.row(1);
...
Eigen::Vector3d TrajectoryPlannerWaypoints::getPosition(Eigen::MatrixXd coeffMatrix, int k, double t) {
    Eigen::Vector3d position(0.0, 0.0, 0.0);
    for (int dim = 0; dim < 3; ++dim) {
        double tn = 1.0;
        for (int i = 0; i < 6; ++i) {
            position(dim) += coeffMatrix.row(k).segment(dim*6,6)(i) * tn;
            tn *= t;
        }//这样写不太好，可能写成矩阵运算更快
    }
    return position;
...
Eigen::Vector3d TrajectoryPlannerWaypoints::getVelocity(Eigen::MatrixXd coeffMatrix, int k, double t) {
    Eigen::Vector3d velocity(0.0, 0.0, 0.0);
    for (int dim = 0; dim < 3; ++dim) {
        double tn = 1.0;
        for (int i = 1; i < 6; ++i) {
            velocity(dim) += coeffMatrix.row(k).segment(dim*6, 6)(i) * i * tn;
            tn *= t;
        }
    }
    return velocity;
...
Eigen::Vector3d TrajectoryPlannerWaypoints::getAcceleration(Eigen::MatrixXd coeffMatrix, int k, double t) {
    Eigen::Vector3d acceleration(0.0, 0.0, 0.0);
    for (int dim = 0; dim < 3; ++dim) {
        double tn = 1.0;
        for (int i = 2; i < 6; ++i) {
            acceleration(dim) += coeffMatrix.row(k).segment(dim*6, 6)(i) * i * (i - 1) * tn;
            tn *= t; 
        }
    }
    return acceleration;
